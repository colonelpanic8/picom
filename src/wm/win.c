...
static double win_animation_output_default(enum win_script_output output) {
	...
	case WIN_SCRIPT_SHADOW_SCALE_Y: return 1.0;
	default: return 0.0;
	}
}
...
static void win_decay_residuals(struct win *w, double delta_t) {
	...
	for (int i = 0; i < NUM_OF_WIN_SCRIPT_OUTPUTS; i++) {
		...
		double target = win_animation_output_default((enum win_script_output)i);
...
bool win_process_animation_and_state_change(struct session *ps, struct win *w, double delta_t) {
	...
	if (w->running_animation_instance) {
		...
		// Save animation output values. If the new animation controls this
		// output, transfer it there; otherwise save to residual storage for
		// smooth decay.
		for (int i = 0; i < NUM_OF_WIN_SCRIPT_OUTPUTS; i++) {
		...
		if (size_changed || position_changed) {
			...
			for (size_t i = 0; i < ARR_SIZE(adjustments); i++) {
				if (new_output_indices[adjustments[i].output] >= 0) {
					new_animation
					    ->memory[new_output_indices[adjustments[i].output]] +=
					    adjustments[i].delta;
				}
			}
			...
			for (size_t i = 0; i < ARR_SIZE(factors); i++) {
				if (new_output_indices[factors[i].output] >= 0) {
					new_animation
					    ->memory[new_output_indices[factors[i].output]] *=
					    factors[i].factor;
				}
			}
		}
